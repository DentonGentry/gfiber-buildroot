...
HOSTAPD_WPA_SUPPLICANT_CONFIG = $(HOSTAPD_DIR)/wpa_supplicant/.config
...
define HOSTAPD_WPA_SUPPLICANT_TLS_CONFIG
	echo "CONFIG_TLS=openssl" >>$(HOSTAPD_WPA_SUPPLICANT_CONFIG)
endef
else
define HOSTAPD_TLS_CONFIG
...
define HOSTAPD_WPA_SUPPLICANT_TLS_CONFIG
	echo "CONFIG_TLS=internal" >>$(HOSTAPD_WPA_SUPPLICANT_CONFIG)
endef
endif
...
define HOSTAPD_WPA_SUPPLICANT_CONFIGURE_CMDS
	echo $(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	cp $(@D)/wpa_supplicant/defconfig $(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	echo "CFLAGS += $(TARGET_CFLAGS) -I$(STAGING_DIR)/usr/include/libnl3/" >>$(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	echo "LDFLAGS += $(TARGET_LDFLAGS)" >>$(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	echo "CC = $(TARGET_CC)" >>$(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	$(SED) "s/^#CONFIG_IEEE80211R/CONFIG_IEEE80211R/" $(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	$(SED) "s/^#CONFIG_DELAYED_MIC/CONFIG_DELAYED_MIC/" $(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	$(SED) "s/^CONFIG_DRIVER_ATMEL/#CONFIG_DRIVER_ATMEL/" $(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	$(SED) "s/^CONFIG_SMARTCARD/#CONFIG_SMARTCARD/" $(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	$(SED) "s/\/local//" $(@D)/wpa_supplicant/Makefile
# Crypto
	echo "CONFIG_CRYPTO=internal" >>$(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	echo "CONFIG_INTERNAL_LIBTOMMATH=y" >>$(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	echo "CONFIG_INTERNAL_LIBTOMMATH_FAST=y" >>$(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	$(HOSTAPD_WPA_SUPPLICANT_TLS_CONFIG)
# Libnl
	$(SED) "s/^#CONFIG_DRIVER_NL80211/CONFIG_DRIVER_NL80211/" $(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	echo "CONFIG_LIBNL32=y" >>$(HOSTAPD_WPA_SUPPLICANT_CONFIG)
# P2P support.
	echo "CONFIG_WPS=y" >>$(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	echo "CONFIG_AP=y" >>$(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	echo "CONFIG_P2P=y" >>$(HOSTAPD_WPA_SUPPLICANT_CONFIG)
	echo "CONFIG_WIFI_DISPLAY=y" >>$(HOSTAPD_WPA_SUPPLICANT_CONFIG)
endef
...
# Misc
	...
	$(SED) "s/^#CONFIG_IEEE80211AC/CONFIG_IEEE80211AC/" $(HOSTAPD_CONFIG)
	...
	$(HOSTAPD_WPA_SUPPLICANT_CONFIGURE_CMDS)
endef

define HOSTAPD_BUILD_CMDS
	$(MAKE) -C $(@D)/$(HOSTAPD_SUBDIR)
# We have to clean src/ for wpa_supplicant.
	$(MAKE) -C $(@D)/src clean
